...
static int lua_string_to_cpu_setp (lua_State *L, int index, cpu_set_t *setp)
{
    ...
    /*
     *  If string begins with 0x, 00, or cstr_to_cpuset() fails
     *   try str_to_cpuset().
     */
    if ((memcmp (s, "0x", 2L) == 0)
            || (memcmp (s, "00", 2L) == 0)
            || (err = cstr_to_cpuset (setp, s))) {
...
static const struct luaL_Reg schedutils_functions [] = {
    { "getaffinity", l_getaffinity      },
    { "setaffinity", l_setaffinity      },
	{ NULL,          NULL               },
};

int luaopen_schedutils (lua_State *L)
{
	luaL_newmetatable (L, "CpuSet");
	luaL_register (L, NULL, cpu_set_methods);

    /*
     *   Create the 'cpuset' subtable for schedutils table.
     *     Add cpuset.SETSIZE member, then cpu_set_functions:
     */
    lua_newtable (L);
    lua_pushnumber (L, CPU_SETSIZE);
    lua_setfield (L, -2, "SETSIZE");
    luaL_register (L, NULL, cpu_set_functions);

    luaL_register (L, "schedutils", schedutils_functions);
